...
## Z_OrderAlgorithms.py
## This file contains all the functions used during the thesis project to
## build the design algorithms, based on building blocks made from Z-order
## curves.
...
is a Z-order curve on top of the two stacks of Z-order curve.
"""
def setupBuildingBlocksUneven(nrNodes, iteration):
    ...
    if(nrBuildingBlocks % 2 != 0):
        for i in range(int(math.floor(nrBuildingBlocks / 2))):
            listOfNodes.extend(Curves.z_orderCurve(iteration, 0, (i + 1) * math.pow(2, iteration)))
            listOfNodes.extend(Curves.z_orderCurve(iteration, math.pow(2, iteration), (math.floor(nrBuildingBlocks / 2) - i) * math.pow(2, iteration)))
        listOfNodes.extend(Curves.z_orderCurve(iteration, math.pow(2, iteration), 0))
    else:
        for i in range(int(math.floor(nrBuildingBlocks / 2))):
            listOfNodes.extend(Curves.z_orderCurve(iteration, 0, (math.floor(nrBuildingBlocks / 2) - i - 1) * math.pow(2, iteration)))
            listOfNodes.extend(Curves.z_orderCurve(iteration, math.pow(2, iteration), (math.ceil(nrBuildingBlocks / 2) - i - 1) * math.pow(2, iteration)))
    return listOfNodes
